...
export function LecturerProfile() {
  ...
  const lecturerProfile = currentLecturer as Record<string, unknown> | undefined;
          ...
          <CardContent className="flex flex-col items-center py-8">
            <Avatar className="size-24 mb-4">
              <AvatarFallback className="text-2xl bg-primary text-primary-foreground">
                {getInitials(lecturerProfile?.firstName as string ?? '')}
              </AvatarFallback>
            </Avatar>
            <h2 className="text-lg font-semibold text-foreground">
              {lecturerProfile?.firstName as string} {lecturerProfile?.lastName as string}
            </h2>
            <p className="text-sm text-muted-foreground capitalize">{user?.role}</p>
            <p className="text-sm text-muted-foreground">{lecturerProfile?.email as string ?? user?.email}</p>
            {lecturerProfile?.department && (
              <p className="text-sm text-muted-foreground mt-1">Department: {String(lecturerProfile.department)}</p>
            )}
          </CardContent>
        </Card>
